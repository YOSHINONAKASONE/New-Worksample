...
結果：**レッド**と出力される。    
`naoko_onepiece = Onepiece.new("ブルー")`⇒今度は「Onepiece」という設計図を使い「naoko_onepiece」と<span>　　　　　　　　　　　　　　　　　　　　</span>インスタンスオブジェクトを作成。このインスタンスで    `puts(naoko_onepiece.color)`　　　　　　　「ブルー」と指定するだけで上記のクラスに情報が埋め込まれる。    
結果：**ブルー**と出力される。
